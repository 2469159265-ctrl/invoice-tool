...
class InvoiceApp(tk.Tk):
    ...
    # ── UI 构建 ─────────────────────────────
    def _build_ui(self):
        title_frame = tk.Frame(self, bg="#1F4E79", pady=12)
        title_frame.pack(fill=tk.X)
        tk.Label(
            title_frame, text="🧾 发票识别统计工具",
            font=("微软雅黑", 18, "bold"), fg="white", bg="#1F4E79"
        ).pack()
        tk.Label(
            title_frame, text="支持 ZIP/RAR/PDF/JPG → Excel  |  视觉模型 + 文本模型",
            font=("微软雅黑", 10), fg="#A9C4E0", bg="#1F4E79"
        ).pack()

        body = tk.Frame(self, bg="#F5F7FA", padx=20, pady=12)
        body.pack(fill=tk.BOTH, expand=True)

        # ── 视觉模型配置 ──
        self._section(body, "🤖 视觉模型（图片识别，用于扫描件/照片）")
        vf = tk.LabelFrame(
            body, text="GLM-4.6V 等视觉模型",
        ...
        # ── 文本模型配置 ──
        self._section(body, "📝 文本模型（PDF有文本层时直接提取，用于清晰PDF）")
        tf = tk.LabelFrame(
            body, text="Qwen3.5 等文本模型",
            bg="#F5F7FA", bd=1, relief="solid",
            fg="#1F4E79", font=("微软雅黑", 9, "bold"), padx=10, pady=6
        )
        ...
        # ── 输入文件 ──
        self._section(body, "📁 输入文件")
        file_row = tk.Frame(body, bg="#F5F7FA")
        ...
        # ── 输出文件 ──
        self._section(body, "💾 输出 Excel")
        out_row = tk.Frame(body, bg="#F5F7FA")
        ...
        # ── 运行按钮 ──
        btn_row2 = tk.Frame(body, bg="#F5F7FA")
        ...
        tk.Label(body, textvariable=self.progress_label,
                 bg="#F5F7FA", fg="#555", font=("微软雅黑", 9)
                 ).pack(anchor="w")

        # ── 日志 ──
        self._section(body, "📋 运行日志")
        self.log_box = scrolledtext.ScrolledText(
            body, height=9, font=("Consolas", 9),
